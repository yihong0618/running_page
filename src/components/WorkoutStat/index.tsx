...
const WorkoutStat = ({value, description, pace, className, distance, onClick, color=MAIN_COLOR}:
    ...
    <span className={`text-3xl font-bold italic`}>{intComma(value)}</span>
    <span className="text-l font-semibold italic">{description}</span>
    { pace && (<span className="text-3xl font-bold italic">{ " " +pace}</span>)}
    { pace && (<span className="text-l font-semibold italic"> Pace</span>)}

    { distance && (<span className="text-3xl font-bold italic">{ " " + distance}</span>)}
