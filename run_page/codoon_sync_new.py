...
def rundata_job(run_data):
    ...
    with open(RUNDATA_FOLDER + "/" + fit_id + ".txt", "w") as f:
...
class Codoon:
    ...
    def get_runs_records(self, page=1):
        ...
        if IS_ONLY_RUN:
            runs = [run for run in runs if run["sports_type"] < 9]
    ...
    def parse_raw_data_to_namedtuple(
        self, run_data, old_gpx_ids, with_gpx=False, with_tcx=False,with_data=False
    ):
        ...
        if with_data:
            rundata_job(run_data)  # RUN DATA part
    ...
    def get_old_tracks(self, old_ids, with_gpx=False, with_tcx=False,with_data=False):
        ...
        #         run_data, old_gpx_ids, with_gpx, with_tcx,with_data
        #     )
        #     if track:
        #         tracks.append(track)
        # return tracks
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="with_data",
        action="store_true",
        help="get all codoon data txt and download",
    )
    parser.add_argument(
        "--with-tcx",
        dest="with_tcx",
    ...
    tracks = j.get_old_tracks(old_tracks_ids, options.with_gpx, options.with_tcx,options.with_data)
